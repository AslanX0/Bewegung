...
def temperatur():
    ...
    if sensor is None:
        print("Fehler: BME680 Sensor nicht verfügbar!")
        return
        ...
        while True:
            if sensor.get_sensor_data():
                ...
                if sensor.data.heat_stable and sensor.data.gas_resistance is not None:
                    ...
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] {output} | Gas: (aufwärmen...)")
            time.sleep(5)
    except KeyboardInterrupt:
        print("\nBeendet")
        GPIO.cleanup()
...
def read_all_sensors():
    """Liest alle Sensordaten und gibt sie als Dictionary zurück."""
    data = {
        'temperature': None,
        'pressure': None,
        'humidity': None,
    ...
    # BME680 Sensordaten
    if sensor:
        for attempt in range(10):  # bis zu 10 Versuche, bis Heater stabil
            if sensor.get_sensor_data():
                ...
                print(f"  BME680: {data['temperature']} °C | {data['pressure']} hPa | "
                      f"{data['humidity']} %RH | Gas: {data['gas_resistance'] if data['gas_resistance'] else '(aufwärmen...)'}")
                break
            time.sleep(1)
        else:
            print("  BME680: Keine Daten nach 10 Versuchen")
    else:
        print("  BME680: Sensor nicht verfügbar")
...
def main_loop():
    ...
    try:
        while True:
            ...
            # In DB speichern, falls Temperatur vorhanden
            if data['temperature'] is not None:
                cursor.execute(
                    """INSERT INTO sensor_data 
                       (timestamp, temperature, pressure, humidity, gas_resistance, movement_detected) 
                       VALUES (?, ?, ?, ?, ?, ?)""",
                    (
                        timestamp,
                        data['temperature'],
            ...
            print(f"  Nächste Messung in {INTERVALL} Sekunden...")
...
def show_last_entries(count=10):
    """Zeigt die letzten Einträge aus der Datenbank."""
    print(f"\n--- Letzte {count} Einträge aus der Datenbank ---\n")
    
    cursor.execute(f"SELECT * FROM sensor_data ORDER BY id DESC LIMIT {count}")
    rows = cursor.fetchall()
    
    if rows:
        print(f"{'ID':<5} {'Timestamp':<20} {'Temp':>8} {'Druck':>10} {'Feucht.':>8} {'Gas':>12} {'Bewegung':<10}")
        print("-" * 80)
        for row in rows:
            movement = "Ja" if row[6] else "Nein"
            gas = f"{row[5]:.0f}" if row[5] else "N/A"
            print(f"{row[0]:<5} {str(row[1]):<20} {row[2]:>7.2f}° {row[3]:>9.2f} {row[4]:>7.2f}% {gas:>12} {movement:<10}")
    else:
        print("Keine Einträge vorhanden.")


# ============================================================================== 
# 6. HAUPTPROGRAMM - MENÜ
# ============================================================================== 
if __name__ == "__main__":
    ...
    print("  4 - Letzte Datenbankeinträge anzeigen")
    print("  0 - Beenden")
